...
public class DiagnosisTreatmentRecordDao implements DiagnosisTreatmentRecordDaoInterface {
    ...
    public DiagnosisTreatmentRecordDao(String ID) {
        ...
        try (InputStream input = new FileInputStream("src/resources/config.properties")) {
            ...
            prop.load(input);
            DIAGNOSISTREATMENTRECORDDB_PATH = prop.getProperty("DIAGNOSISTREATMENTRECORDDB_PATH",
                    "src/data/DiagnosisTreatmentRecords");
    ...
    public boolean updateDiagnosisTreatmentRecord(String diagnosis, String prescription, String treatmentPlan,
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(diagnosisTreatmentRecordFile));
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length == 8) {
                    String diagnosisTreatmentRecordId = parts[0].trim();
                    String appointmentId = parts[1].trim();
                    String doctorID = parts[2].trim();
                    String appointmentDateStr = parts[3].trim();
                    String appointmentTimeSlot = parts[4].trim();
                    String existingDiagnosis = parts[5].trim();
                    String existingPrescription = parts[6].trim();
                    String existingTreatmentPlan = parts[7].trim();
                    ...
                    pw.println(String.join(",", diagnosisTreatmentRecordId, appointmentId, doctorID, appointmentDateStr,
                            appointmentTimeSlot, existingDiagnosis, existingPrescription, existingTreatmentPlan));
                } else {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length == 8) {
                    ...
                    if (appointmentDateStr.equals(date.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")))) {
                        ...
                        Appointment appointment = new Appointment("confirmed", appointmentDateStr, startTimeStr,
                                endTimeStr, null, doctorID);

                        // Create and return the DiagnosisTreatmentRecord object
                        diagnosisRecord = new DiagnosisTreatmentRecord(UUID.fromString(diagnosisTreatmentRecordId),
                                existingDiagnosis, existingPrescription, existingTreatmentPlan, appointment);
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(diagnosisTreatmentRecordFile))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length == 8) // Ensure the correct number of columns
                {
                    ...
                    // Create an Appointment object
                    Appointment appointment = new Appointment("confirmed", appointmentDateStrTrimmed, startTimeStr,
                            endTimeStr, patientID, doctorID);

                    // Create a DiagnosisTreatmentRecord object
                    DiagnosisTreatmentRecord treatRec = new DiagnosisTreatmentRecord(
                            UUID.fromString(diagnosisTreatmentRecordId),
                            existingDiagnosis, existingPrescription, existingTreatmentPlan, appointment);
    ...
    public void addDiagnosisTreatmentRecord(DiagnosisTreatmentRecord treatRec) {
        ...
        try (
            ...
            String newRecord = appointment.getDoctorID() + "," + appointment.getAppointmentDate() + "," +
                    timeSlot.getStartTime().toString() + " - " + timeSlot.getEndTime().toString() + "," + diagnosis
                    + "," + prescription +
                    "," + treatmentPlan;
